...
@task(name="read_dataframe", description="Lectura del set de datos", retries=3, retry_delay_seconds=10, log_prints=True)
def read_dataframe():
...
@task(name="normalizar_log", description="Normalizacion a logaritmo", retries=3, retry_delay_seconds=10, log_prints=True)
...
@task(name="concatenar", description="Concatenacion de dataframes", retries=3, retry_delay_seconds=10, log_prints=True)
def concatenar(datos, continuas_L):
    datos_concat = pd.concat([datos, continuas_L], axis=1)
    return datos_concat

@task(name="PCA_generation", description="Generacion de componentes principales", retries=3, retry_delay_seconds=10, log_prints=True)
def PCA_generation(explicatorias):
    ...
    cumVar = pd.DataFrame(np.cumsum(res_pca.explained_variance_ratio_) * 100, columns=["cumVarPerc"])
    expVar = pd.DataFrame(res_pca.explained_variance_ratio_ * 100, columns=["VarPerc"])
    pd.concat([expVar, cumVar], axis=1).rename(index={0: "PC1", 1: "PC2", 2: "PC3", 3: "PC4", 4: "PC5", 5: "PC6", 6: "PC7", 7: "PC8", 8: "PC9", 9: "PC10"})
    return componentes

@task(name="concatenar_PCA", description="Concatenacion de componentes principales", retries=3, retry_delay_seconds=10, log_prints=True)
def concatenar_PCA(datos_final, componentes):
    datos_concat = pd.concat([datos_final, componentes], axis=1)
    return datos_concat

@task(name="split_data", description="Particion de datos", retries=3, retry_delay_seconds=10, log_prints=True)
...
@task(name="train_model_PCA", description="Entrenamiento del modelo con PCA", retries=3, retry_delay_seconds=10, log_prints=True)
...
@flow(name="Modelo Propinas", description="End-to-end ML pipeline for tips prediction", log_prints=True)
def tip_prediction():
    datos = read_dataframe()
    datos.head()

    continuas_L = normalizar_log(datos)
    datos_final = concatenar(datos, continuas_L)
    print(datos_final.head())

    # PCA solo con features. Se elimina tip_L del PCA.
    explicatorias = datos_final[["total_bill_L", "size_L"]]

    componentes = PCA_generation(explicatorias)
    datos_final = concatenar_PCA(datos_final, componentes)
    print(datos_final.head())

    datosEntrenamiento = split_data(datos_final)
...
if __name__ == "__main__":
    # Asegura ver los runs en el UI conectando a tu server:
    # export PREFECT_API_URL=http://127.0.0.1:4200/api   (PowerShell: $Env:PREFECT_API_URL="http://127.0.0.1:4200/api")
    tip_prediction()
